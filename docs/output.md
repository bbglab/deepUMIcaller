...
- Having the `amfiltered` BAM file in `processing_files/sortbamamfiltered/` allows you to merge sequencing data from the same sample coming from different libraries and do the calling together.

### Outputs

- duplex_reads_bam
- processing_files/sortbamamfiltered
...
![FamilyMetricsPlotsIDT](images/family_metrics_idt.png)

a-d. Distribution of family sizes of unique molecules in 4 DNA duplex libraries prepared from
different samples. Unique molecules are separated depending on whether (red) or not (black)
they form duplex consensus reads. A vertical blue line represents the mode of the duplex
consensus family sizes. The value of this mode (family size peak) is presented within the legend
box. Numbers at the right side of the plots present key metrics of the duplex consensus building
process. These plots are generated by the deepUMIcaller family metrics module (see
FAMILYMETRICS deepUMIcaller step 12b,d in the Supp. Note 1). While family size distributions
with mode above 4 and from a library with duplicate rate above 75% are deemed good, we
normally request more sequencing from libraries with mode below 10 (as the case shown in panel
a).
...
- sigprofiler/all
- sigprofiler/purine
- sigprofiler/pyrimidine

### Examples

#### Mutations per position QCs

![Mutations_per_position_QCs](images/mutation_per_position.png)

a,b. Number of mutations identified at each position of their supporting reads, colored by
nucleotide change type. The black line represents the fraction of mutations identified at each
position.The heatmaps represent the ratio of the mean number of mutations in the first 5 bps of
the read divided by the mean number of mutations per bp in the rest of the positions of the read.
These are computed separately for each possible mutation type and are plotted in the same order
as the legend of the plot. Values greater than 2 indicate the need to carry out further clipping, or
discarding the sample if clipping is not enough to correct the issue. These plots are generated by
deepUMIcaller (see deepUMIcaller Supp. Note 1, steps 43 and 44; and they are stored in
metrics/mutations/ directory). In both cases deepUMIcaller was run with a default trimming of 10
bps at the beginning of the reads.
