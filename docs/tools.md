...
- **All-molecules filter** (`FILTERCONSENSUSREADSAM`, default `--min-reads 2 1 0`, no strand agreement required) — keeps both duplex and single-strand consensus reads; produces the BAM used to compute `VAF_AM` and `VAF_ND` at postprocessing time.
